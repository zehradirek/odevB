...
public class ProductService {
	...
	public List<ProductResponseDTO> getAllProducts() {
		...
		List<ProductResponseDTO> responseDTO = products.stream()
				.map(product -> this.modelMapperConfig.forResponse().map(product,
	...
	public ProductResponseDTO getProductById(int productId) {
		...
		ProductResponseDTO responseDTO = this.modelMapperConfig.forResponse().map(product,
				ProductResponseDTO.class);
		return responseDTO;
	}

	public ProductResponseDTO createProduct(ProductRequestDTO productRequestDTO) {
		Product newProduct = this.modelMapperConfig.forRequest().map(productRequestDTO,
				Product.class);
	...
	public void updateProductDescription(String productDescription) {
		Optional<Product> oProduct = productRepository.findByProductDescription(productDescription);
